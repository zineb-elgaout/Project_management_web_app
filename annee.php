...
// Debug
error_log("Accès page par date - User: ".$_SESSION['id_utilisateur']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_remarque'])) {
    ...
    if ($stmt->rowCount() === 1) {
        ...
        header("Location: ".$_SERVER['PHP_SELF']."?annee=".urlencode($selectedAnnee)."&mois=".urlencode($selectedMois));
        exit();
    } else {
...
try {
    ...
    $query = "SELECT 
      ...
      WHERE p.id_enseignant = :id_enseignant"; // Filtre par encadrant
    
    // Ajouter les conditions pour l'année et le mois si sélectionnés
    $params = [':id_enseignant' => $id_enseignant];
    $conditions = [];
    
    if (!empty($selectedAnnee)) {
        $conditions[] = "YEAR(p.date_soumission) = :annee";
        $params[':annee'] = $selectedAnnee;
    }
    
    if (!empty($selectedMois)) {
        $conditions[] = "MONTH(p.date_soumission) = :mois";
...
<head>
    ...
    <title>Synergia - Recherche par Date</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a href="#" class="nav-link active"><i class="fas fa-calendar-alt me-2"></i>Par date</a>
    ...
    <div class="main-content">
        ...
        <div class="container py-5 flex-grow-1">
            <div class="row mb-4">
                <div class="col-12">
                    <h2 class="fw-bold">Projets par Date de Soumission</h2>
                    <p >Filtrez et consultez les projets par année et mois de soumission</p>
                </div>
            </div>
            ...
            <?php else: ?>
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4>
                        ...
                        <?php if (empty($selectedAnnee) && empty($selectedMois)) echo "Tous les projets"; ?>
                    </h4>
